...
SELECT p.id, u.screenname as author, p.title, p.preview, p.url, p.text, p.tags, p.timestamp
  FROM posts p
...
SELECT p.id, u.screenname as author, p.title, p.preview, p.url, p.tags, p.timestamp
  FROM posts p
  JOIN users u on p.author = u.id
